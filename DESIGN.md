...
- **朱赤 Vermilion Accent** `oklch(0.58 0.15 32)`（暗色模式 `oklch(0.64 0.155 33)`）：占任意一屏 ≤10% 面积，承担唯一强调 —— 坐席图标注点（**所有**标注点的身份色，不止选中态，见下方 The Pin Vermilion Rule）、关键 CTA 在按下时的状态、上传成功的颗粒高光。OKLCH chroma 控制在中等饱和（参考 0.10-0.18），不允许霓虹感。
...
**The Pin Vermilion Rule（issue #7 例外）.** 坐席图上的**所有**用户标注点（不只是选中态）都用朱赤实心核 + 纸色 halo 环（`border-background`，非 box-shadow）。这是对上面「Accent 绝不可日常使用、稀有性就是意义」的一处**刻意例外**：真实坐席图是以蓝色为主的彩色插画底图，原本「纸色填充 + 淡朱赤描边 + 10px」那种"沉进纸面"的安静默认点在上面几乎隐形（深色模式下是看不清的小黑点）；而标注点是本产品的核心价值（"这些座位有视角图"），值得这抹颜色。选中态**不靠颜色**区分，而靠：更大（size-4）+ 墨色描边（`border-foreground`）+ 朱赤微环（`ring-accent/45`），守住 non-color-only。聚合计数气泡反过来**保持中性**（奶色底 + 墨调对比环），形成「朱赤=单个标点、中性=聚合数量」的层次。pin 本身极小，朱赤总占屏仍 ≤10%。
