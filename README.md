...
npm run build
npm run preview          # wrangler dev → 上传 / 暂存 / 后台 API 全部可用
```

> 选哪个？写 UI、调样式用 `npm run dev`（最快）。要联调上传 / 暂存 / 后台等需要 Cloudflare 绑定的功能时，用 `npm run build && npm run preview`（`wrangler dev` 通过 `platformProxy` 提供本地 D1/KV/R2）。
...
| `npm run preview` | `wrangler dev`，本地跑构建产物 + 绑定 |
