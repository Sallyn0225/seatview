...
> 티켓을 잡고 좌석을 고르기 전에, 그 좌석에서 실제로 무엇이 보이는지 먼저 확인하세요.
> リアル座席ビュー · 真实视角图集 —— 내부 코드네임 `seatmap-real`

**SeatView** 는 일본(일부 해외 포함) 콘서트 공연장의 **실제 좌석 시야 사진**을 모읍니다. 사용자는 공연장 공식 좌석도 위에 자신의 좌석을 표시하고 그 위치에서 찍은 실사 사진을 업로드합니다. 다른 사람은 좌석도의 마커를 클릭하면 그 좌석의 실제 시야를 Lightbox 에서 미리 볼 수 있어, 티켓을 잡거나 좌석을 고르기 전에 더 현명한 결정을 내릴 수 있습니다.
...
소프트 삭제: 메인테이너가 `/admin` 에서 소프트 삭제하면 D1 이 `deleted_at` 을 설정하고(공개 쿼리는 `deleted_at IS NULL` 로 필터링하므로 마커 / 카드가 즉시 사라집니다) R2 객체를 물리적으로 삭제합니다.
